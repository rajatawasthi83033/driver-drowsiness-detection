...
export function StatusCard({ status, isConnected }: StatusCardProps) {
  const state = status?.state || 'Safe';
  ...
  return (
    ...
    >
      ...
      <div className={cn('flex flex-col items-center py-6', config.bg, 'rounded-lg')}>
        {/* Status Indicator */}
        <div className="relative">
          <div
            className={cn(
              ...
              config.border,
              isConnected && status && 'animate-pulse-glow',
              config.color
            )}
          >
          ...
          {isConnected && status && (
            <div
              className={cn(
                ...
                state === 'Safe' && 'bg-green-500',
                state === 'Drowsy' && 'bg-yellow-500',
                state === 'Dangerous' && 'bg-red-500'
              )}
            />
          )}
        </div>

        <p className={cn('mt-4 text-sm text-center', config.color)}>
          {config.description}
        </p>
      </div>

      {/* Sleep Risk Indicator */}
      {status?.sleep_risk && (
